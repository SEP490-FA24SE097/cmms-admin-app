...
export default function StoreDashboard() {
    ...
    const { data: dashboardDataResponse, isLoading, error } = useGetDashboard(dashboardQueryParams);
    ...
    const groupedData = dashboardData.reduce((acc, store) => {
        const { storeId, storeName, month, totalRevenue, totalRefunds, profit, totalInvoices } = store;
        if (!acc[storeId]) {
            acc[storeId] = {
                storeId,
                storeName,
            ...
            <div className="row-span-9 mt-3 w-[80%] mx-auto">
                ...
                <div style={{ marginBottom: "20px" }}>
                    ...
                    <select
                        ...
                        onChange={(e) => setDashboardQueryParams({ Year: Number(e.target.value) })}
                ...
                {Object.values(groupedData).map((store) => (
                    <div key={store.storeId} style={{ marginBottom: "2rem", border: "1px solid #ccc", borderRadius: "8px", padding: "20px", backgroundColor: "#f9f9f9" }}>
                        <h3 className="text-center font-bold">{store.storeName}</h3>
                        <div className="flex"><FaFileInvoiceDollar className="size-7 mr-3" />{store.totalInvoices} đơn hàng</div>
                        <div className="flex"><  RiHandCoinFill className="size-7 mr-3" />{(store.totalRevenue)}vnđ doanh thu</div>
                        {/* Biểu đồ */}
